...
public class Puzzle {   
     ...
     */
    public Puzzle(int h, int w) throws puzzle.PuzzleExceptions.ConstructorsExceptions {
        
        try {
            if (h < 1 || h > 500 || w < 1 || w > 500) throw new PuzzleExceptions.ConstructorsExceptions(PuzzleExceptions.ConstructorsExceptions.NO_BOARDS_HW_NEGATIVE);
            this.h = h; 
            this.w = w;
...
// Methods for moving groups
    ...
    public void makeVisible() throws PuzzleExceptions.makeVisibleInvisibleExceptions{
        try{
            if(starting == null) throw new PuzzleExceptions.makeVisibleInvisibleExceptions(PuzzleExceptions.makeVisibleInvisibleExceptions.NO_STARTING_BOARD_NULL);
            if(ending == null) throw new PuzzleExceptions.makeVisibleInvisibleExceptions(PuzzleExceptions.makeVisibleInvisibleExceptions.NO_ENDING_BOARD_NULL);
    ...
    public void makeInvisible() throws PuzzleExceptions.makeVisibleInvisibleExceptions{
        try{
            ...
            startingBoard.makeInvisible();  // Make visible the initial board
            endingBoard.makeInvisible();    // Make visible the final board
